...
import type { PdfAppointment, CaseFile } from "@/types";
...
// Check if entry matches source
function matchesEntry(
    sourceEntry: PdfAppointment | undefined,
    entry: PdfAppointment
): boolean {
        ...
        <Card className="border border-emerald-200 bg-emerald-50/70">
            <CardHeader className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                <CardTitle>
                    ...
                    {pdfDate && (
                        <span className="ml-2 text-sm text-emerald-700 font-normal">
                            ({pdfDate})
                        </span>
                    )}
                </CardTitle>
                        ...
                        <table className="min-w-full text-xs md:text-sm">
                            ...
                            <tbody>
                                {pdfEntries.map((entry) => {
                                    // Check if this entry is assigned
                                    let isAssigned = false;
                                    if (cases || history) {
                                        const inCases =
                                            cases?.some((c) =>
                                                matchesEntry(c.sourcePdfEntry, entry)
                                            ) || false;
                                        const inHistory = history
                                            ? Object.values(history).some((dayCases) =>
                                                dayCases.some((c) =>
                                                    matchesEntry(c.sourcePdfEntry, entry)
                                                )
                                            )
                                    ...
                                    return (
                                        <tr
                                            key={entry.id}
                                            className={`border-b last:border-b-0 ${selectedPdfEntryId === entry.id
                                                    ? "bg-emerald-50"
                                                    : "bg-white"
                                                } ${isAssigned ? "relative opacity-75" : ""}`}
                                        >
                                            {isAssigned && (
                                                <div className="absolute inset-0 pointer-events-none z-10">
                                                    <div className="absolute top-1/2 left-0 right-0 h-1 bg-red-500 transform -translate-y-1/2 shadow-sm" />
                                                </div>
                                            )}
                                            <td
                                                className={`p-2 font-semibold relative z-0 ${isAssigned ? "text-red-600" : ""
                                                    }`}
                                            >
                                                {entry.time}
                                            </td>
                                            <td
                                                className={`p-2 relative z-0 ${isAssigned ? "text-red-600" : ""
                                                    }`}
                                            >
                                                {entry.name}
                                            </td>
                                            ...
                                            <td
                                                className={`p-2 text-xs text-slate-600 relative z-0 ${isAssigned ? "text-red-600" : ""
                                                    }`}
                                            >
                                                {entry.extra || "-"}
                                            </td>
